...
import com.ljheee.socket.Client;
 ...
 *
 */
public class LoginFrameClient extends JFrame {
	
	private TextField textFieldAccount = null; //账户
	static TextField textFieldPass = null;	//密码
	private ImageIcon dlGUI=new ImageIcon(LoginFrameClient.class.getResource("/dl.png")); //要显示的  背景图层
	...
	public LoginFrameClient() {
		...
		button.setIcon(new ImageIcon(LoginFrameClient.class.getResource("/dlbtn.png")));
...
//				List usernames = DbUtil.getUserNames();
		 		Map map = DbUtil.getUsers();
		 		int userAcount = Integer.parseInt(textFieldAccount.getText());
		 		
		 		//登陆成功
		 		if(map.containsKey(userAcount)&&map.get(userAcount).equals(textFieldPass.getText())){
		 			disposeFrame();  // "关闭"当前Login界面
					new Client(""+userAcount);	//启动client，
		 		}else if(textFieldAccount.getText().equals("") || textFieldPass.getText().equals("")){
					JOptionPane.showMessageDialog(null, "账户/密码为空");    //窗口：提示消息
				}else{ 
		 ...
		 //背景图片，添加到背景Panel里面
		 JLabel bgLabel = new JLabel(new ImageIcon(LoginFrameClient.class.getResource("/dl.png")));
		 ...
		 closeButton.setIcon(new ImageIcon(LoginFrameClient.class.getResource("/closebtn.png")));
		...
		//处理拖动事件(去掉边框后，实现JFrame的拖动)
			...
			public void mousePressed(MouseEvent e) {
			  ...
			  LoginFrameClient.this.setLocation(xx, yy);//设置拖拽后，窗口的位置
			  }
			 });
...
//			 btnNewButton.setContentAreaFilled(false);
			 ...
			 btnNewButton.addActionListener(new ActionListener() {
			 	public void actionPerformed(ActionEvent arg0) {
			 		//注册
			 		List usernames = DbUtil.getUserNames();
			 		Map<String, String> map = DbUtil.getUsers();
			 		
			 		if(textFieldAccount.getText().equals("") || textFieldPass.getText().equals("")){
			 			JOptionPane.showMessageDialog(LoginFrameClient.this, "账户/密码为空");    //窗口：提示消息
			 		}else if(usernames.contains(textFieldAccount.getText())){
			 			JOptionPane.showMessageDialog(LoginFrameClient.this, "用户名已存在");
			 		}else{
			 			//新注册用户
			 			String acount = textFieldAccount.getText();
			 			String pass = textFieldPass.getText();
			 			
			 			//插入数据库
			 			boolean b =DbUtil.insertUser(Integer.parseInt(acount),pass);
			 			if(b==false){
			 				JOptionPane.showMessageDialog(LoginFrameClient.this, "注册成功");
			 			}
			 		}
			 	}
			 });
			 springLayout.putConstraint(SpringLayout.NORTH, btnNewButton, 0, SpringLayout.NORTH, textFieldAccount);
			 springLayout.putConstraint(SpringLayout.WEST, btnNewButton, 6, SpringLayout.EAST, textFieldAccount);
	...
	/**
	 * dispose登录界面[相当于隐藏了]，然后再去显示跳转的界面
	 */
	private void disposeFrame() {
		LoginFrameClient.this.dispose();
	}
